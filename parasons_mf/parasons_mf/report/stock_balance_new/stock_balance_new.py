	...
	def apply_items_filters(self, query, item_table) -> str:
		...
		for field in ["item_code","brand"]:
			if not self.filters.get(field):
				...
				if len(self.filters.get("item_code")) > 1:
					item_code = tuple(self.filters.get("item_code"))
					query = query.where(item_table.name.isin(item_code))
				elif len(self.filters.get("item_code")) == 1:
					item_code = (self.filters.get("item_code")[0])
